...
--https://github.com/churichard/notabase/issues/1178
...
CREATE TYPE public.plan_id AS ENUM ('basic', 'pro', 'catalyst');

DROP TYPE IF EXISTS subscription_status CASCADE;
CREATE TYPE public.subscription_status AS ENUM ('active', 'inactive');

DROP TYPE IF EXISTS billing_frequency CASCADE;
CREATE TYPE public.billing_frequency AS ENUM ('monthly', 'annual', 'one_time');
...
CREATE OR REPLACE FUNCTION create_onboarding_notes()
  ...
  as $$
    declare
      getting_started_id uuid := gen_random_uuid();
      linked_note_id uuid := gen_random_uuid();
      stack_note_id uuid := gen_random_uuid();
    begin
